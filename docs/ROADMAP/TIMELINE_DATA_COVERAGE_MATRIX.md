...
| Divided Kingdom | Preview | None | Expanded Preview | Preview | Partial | World context labels only / Korean placeholder | Event rows lag behind kingdom comparison density; prophets not broadly covered; package row model not yet implemented | Use the Kings / Kingdoms package design to define future king, kingdom, and transition skeleton rows |
...
| Kings / Kingdoms | Comparison of rulers, kingdoms, prophets, empires | `timelineKingdomComparisonRows` | Expanded Preview in frontend; package design completed; skeleton/frontend package integration pending | Strongest non-event supporting table; inspector cross-links exist; future package direction is now documented | Full king sequence and prophet coverage remain incomplete; no package-backed skeleton or route loader yet | Build the skeleton package before preview integration |
