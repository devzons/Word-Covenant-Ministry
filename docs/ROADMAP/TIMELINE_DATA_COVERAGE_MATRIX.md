...
| United Kingdom | Preview | Preview | Expanded Preview | Preview | Partial | World context labels only / Korean placeholder | Event count is modest relative to supporting tables | Balance event rows with existing kingdom and psalm context |
| Divided Kingdom | Preview | None | Expanded Preview | Preview | Partial | World context labels only / Korean placeholder | Event rows lag behind kingdom comparison density; prophets not broadly covered | Expand event and book context before more label-only links |
...
| Books / Psalms | Book background and Psalm context | `timelineBookContextRows` plus `books.66-canonical-skeleton.json` | Expanded Preview in frontend; 66-book package preview connected | Clear book-context cards; package-level 66-book skeleton now renders as a metadata-only preview; left canonical section guide now targets stable center sections | Psalms and book intros remain minimal; event package is still not imported into frontend | Keep runtime fixture stable while planning deeper package integration |
