...
| Kings / Kingdoms | Comparison of rulers, kingdoms, prophets, empires | `timelineKingdomComparisonRows` plus `kings-kingdoms.skeleton.json` | Expanded frontend preview; skeleton package created; frontend package integration pending | Strongest non-event supporting table; inspector cross-links exist; package design and first skeleton baseline now exist | Full king sequence and prophet coverage remain incomplete; no route loader or package-backed frontend preview yet | Add verifier fixtures and harden the package path before frontend preview integration |
