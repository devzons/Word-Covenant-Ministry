...
| Events | Primary narrative timeline grouped by period | `events.core-biblical-skeleton.json` via `TimelinePageShell.tsx` route loader | Expanded Preview in frontend; 85-row core event package preview connected; schematic highlight baseline connected; dedicated QA recorded in `CORE_EVENT_FRONTEND_QA.md` | Period grouping, event selection, Scripture Evidence Panel integration, a package-backed Genesis-to-Revelation baseline, same-period/section highlight derived from current selection, explicit metadata-only / reference-only guardrails, and verifier-wrapper coverage confirmed in QA | Detailed sub-event coverage and denser relation metadata still remain future work; live route smoke was unavailable from this environment; build verification was environment-limited by Turbopack process/port binding restriction rather than a confirmed Events-preview code defect | Keep the preview metadata-only while expanding event depth cautiously only after separate approved content work |
...
| Kings / Kingdoms | Comparison of rulers, kingdoms, prophets, empires | `timelineKingdomComparisonRows` plus `kings-kingdoms.skeleton.json` | Expanded frontend preview; skeleton package created; verifier fixtures created; verifier hardening complete; package-backed frontend preview connected; interaction QA completed; schematic highlight baseline connected; metadata-only cross-link refinement recorded; deeper-expansion scope gate documented | Strongest non-event supporting table; inspector cross-links exist; package design, skeleton baseline, fixture baseline, Kings-specific verifier checks, package-backed frontend preview, left section navigation, center accordion behavior, explicit relation highlighting, related event chips from package metadata, and caution-gated chronology emphasis are now in place | Full king sequence and prophet coverage remain incomplete; prophet labels remain supporting tags rather than standalone selectable rows; chronology remains intentionally cautious and metadata-only; live route smoke remained unavailable in this environment; current data source remains mixed because runtime comparison rows still coexist with the package baseline | Keep expansion metadata-only and require separate row-level authorization before any deeper package-row additions |
| Genealogy | Matthew-oriented comparison rows | `timelineGenealogySegments`, `timelineGenealogyComparisonRows` | Preview | Clear Matthew 14 / 14 / 14 framing; explicit row linking exists; copy now makes the structure-observation boundary and no-exact-chronology intent explicit | Luke genealogy and broader OT genealogies are absent; still runtime fixture-backed rather than package-backed | Keep the current Matthew 1 preview metadata-only while deferring wider genealogy model work |
| Places / Schematic Map | Concept-zone place reference without coordinates | `timelineSchematicPlaceRows` | Preview with schematic placeholder highlight; final QA recorded; no-coordinate wording refined | Schematic concept-zone approach is clear; no coordinate overclaim; the center column now includes a schematic-only placeholder highlight summary derived from current selection and existing metadata; no coordinates, map provider, geocoding, or route reconstruction are introduced; final QA addendum is recorded in `SCHEMATIC_FLOW_HIGHLIGHTS_QA.md` | Many biblical places missing; no gospel/Acts place spread; still no package-backed places dataset; placeholder remains summary-only rather than full relation rendering | Keep future highlights schematic-only and metadata-derived before expanding place coverage |
| Korean History Reference placeholder | Supporting-only reference section under event periods | `references.korean-pilot.json` via `ScriptureTimelineList.tsx` | Pilot preview | Authority boundary is explicit; default collapsed; supporting-only pilot rows now render where period mapping exists; intentionally post-biblical pilot rows can remain outside direct biblical-period linkage; inspector-policy boundary is now documented | No inspector integration yet; no search/filter participation; broader expansion still requires careful assignment policy and a separate implementation CR | Keep direct biblical-period assignment narrow, preserve intentional post-biblical non-linking, and require a secondary-only inspector policy before any future right-panel work |
...
| Prophets | Samuel, Nathan, Elijah, Isaiah, Jeremiah, Hosea, Amos appear as support tags in places, events, or kingdom rows; current inspector copy now keeps those labels clearly secondary and non-selectable | Many prophets absent; cross-links remain selective; no standalone prophet row model is active | Expand only with explicit row-level support |
...
- `4` pilot rows currently use broad `relatedBiblicalPeriodIds`, while `korean-three-kingdoms-formation-reference` remains intentionally outside direct biblical-period assignment because it is a post-biblical supporting reference.
