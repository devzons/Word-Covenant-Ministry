...
| Scripture Evidence Panel / Context Inspector | Right-side contextual evidence and cross-linking | Existing preview collections via lookup maps | Expanded Preview | Can navigate among existing Events, Books / Psalms, Places, Kingdoms, and explicit Genealogy rows; CR-91C deep-link scope is now documented | Quality depends on sparse underlying data; URL-driven deep-link restore is still implementation-pending | Implement CR-91C-2 deep links for Events, Books / Psalms, and Kings / Kingdoms while keeping the current metadata-only guardrails |
