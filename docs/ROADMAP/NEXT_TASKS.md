...
13. Recommended next CR: `CR-BR-CTX-28 Chapter Context Verifier Implementation Readiness`. This should remain docs-only and compare the new chapter-context fixture inputs against the current verifier architecture and wrapper expectations before any verifier code change begins. It should continue to defer verifier code changes, production chapter rows, Reader runtime hookup, verse-level tagging, selected-verse-driven filtering, person/paja/original-language name atlas work, place/genealogy/Korean-reference deep links, map/coordinates/geocoding/provider work, Korean/world-history inspector integration, Timeline Kingdom extraction, and all API / backend / DB / schema / Timeline data package row changes.
