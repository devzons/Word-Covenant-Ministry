...
1. `Scripture Evidence Panel Completeness Fixes A` is now complete through available validation. The Events, Books / Psalms, and Kings / Kingdoms right-panel surfaces now preserve and render more existing package-backed metadata without adding data rows, entity types, APIs, schemas, coordinates, or external-reference layers. Existing source/package fields such as event confidence, event related kingdom/event IDs, book basis/confidence labels, book related book/kingdom IDs, and kings package basis metadata are now carried through and shown more clearly in the existing Scripture Evidence Panel structure. Bible text remains unrendered, chronology remains caution-labeled supporting metadata rather than exact proof, and no API, DB, backend, schema, or runtime import/export work was added. The next recommended implementation CR should focus on `Places panel basis/confidence clarity` as a separate small frontend-only step while person, paja/name, Korean/world-history inspector integration, prophet-context expansion, and map work remain deferred.
2. `Scripture Evidence Panel Safe Completeness Fix Plan` is now documented in `docs/ROADMAP/SCRIPTURE_EVIDENCE_PANEL_SAFE_COMPLETENESS_FIX_PLAN.md`. This plan narrowed the implementation-safe scope to frontend-only and normalizer-only carry-through fixes using existing source/package fields without adding rows, entities, APIs, schemas, coordinates, or external-reference layers. Its first implementation step for Events, Books / Psalms, and Kings / Kingdoms is now complete; Places panel clarity and broader wording standardization remain separate follow-up CRs.
3. `Scripture Evidence Panel Modular Triage Plan` is now documented in `docs/ROADMAP/SCRIPTURE_EVIDENCE_PANEL_MODULAR_TRIAGE_PLAN.md`. The current audit result has now been translated into implementation-safe categories: rendering gaps, normalizer gaps, source-data gaps, intentional preview limitations, and approval-required expansion. The future right-side panel is being reframed as a modular context surface for the broader `Scripture Context Atlas`, but this triage step still authorizes no implementation. The next safe implementation-facing step after Fixes A is a narrow frontend-only Places clarity pass followed by wording standardization and later modular component design.
4. `Scripture Context Atlas Redesign Scope` is now documented in `docs/ROADMAP/SCRIPTURE_CONTEXT_ATLAS_REDESIGN_SCOPE.md`. The current Timeline workspace has a validated metadata baseline across Events, Books / Psalms, Kings / Kingdoms, Genealogy, Places, and supporting Korean-history references, but the product direction is now being reframed toward a Scripture-reading-first `Scripture Context Atlas`. The redesign keeps Bible text primary, reuses existing Timeline data as right-panel context supply, keeps map work schematic-only and no-coordinate, keeps original-language / paja work subordinate to Scripture context, and defers implementation. The required pre-prototype panel audit and safe-fix planning are now documented, and the current next small implementation target is Places-panel clarity rather than reader-first layout work.
