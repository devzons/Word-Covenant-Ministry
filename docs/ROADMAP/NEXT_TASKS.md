...
3. `CR-BR-CTX-17 Reader Context Panel Advanced Timeline Link Audit and Design` is now complete as a docs-only readiness gate in `docs/ROADMAP/READER_CONTEXT_PANEL_TIMELINE_LINK_DESIGN.md`. The current recommended next implementation direction is to keep Reader related-metadata chips non-interactive while adding only secondary advanced-Timeline links for supported targets. Current safe targets are limited to `event`, `book`, and `kingdom`, because the Timeline route already restores those inspect types from query params. Place, genealogy, and Korean-history reference links from the Reader remain deferred until Timeline deep-link restore and UX policy broaden in a later approved step.
4. `CR-BR-CTX-18 Reader Context Panel Advanced Timeline Link Implementation` is now the recommended next reader-first implementation CR. It should stay frontend-only, preserve book-level metadata-only boundaries, add secondary `Open in advanced Timeline` actions only for supported related metadata targets, and keep chips themselves non-interactive. It must not add reader-side entity resolution, verse-level tagging, place/genealogy/Korean-history deep links, API / backend / DB / schema changes, Timeline data package row changes, or Timeline Kingdom extraction work.
