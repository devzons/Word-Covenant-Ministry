...
17. `CR-BR-CTX-48 Chapter Context Pilot Draft Row QA` is now complete as a QA-only pass on the current `4` pilot draft rows. The QA result is pass with no row edits required, verifier still passing, warnings at `0`, and all rows still remaining `reviewStatus: "draft"` with `isSkeleton: false`. No Reader/runtime hookup, verifier changes, fixture changes, verse-level tagging, selected-verse-driven filtering, person/paja/original-language name atlas work, place-package or map/coordinates/geocoding/provider work, Timeline Kingdom extraction, or API / backend / DB / schema changes were added.
18. `CR-BR-CTX-49 Chapter Context Draft-to-Review Gate Design` is now complete as a docs-only gate-definition step in `docs/ROADMAP/CHAPTER_CONTEXT_DRAFT_TO_REVIEW_GATE_DESIGN.md`. The chapter-context track now has explicit author self-check criteria, promotion conditions for `review-required`, blocker conditions, and draft-only correction rules, while keeping the current `4` pilot rows unchanged at `reviewStatus: "draft"`. This step does not authorize row promotion, package maturity changes, Reader/runtime hookup, verifier or fixture changes, verse-level tagging, selected-verse filtering, place-package work, Timeline Kingdom extraction, or API / backend / DB / schema changes.
19. Recommended next CR: `CR-BR-CTX-50 Chapter Context Author Self-Check Execution`. This should apply the new self-check gate to the current `4` pilot draft rows one by one, keep any non-passing row at `draft`, and allow promotion to `review-required` only where the author self-check fully passes. It should continue to defer Reader/runtime hookup, verifier code changes, fixture changes, verse-level tagging, selected-verse-driven filtering, person/paja/original-language name atlas work, place-package or map/coordinates/geocoding/provider work, Timeline Kingdom extraction, and all API / backend / DB / schema changes.
