...
4. `CR-BR-CTX-18 Reader Context Panel Advanced Timeline Link Implementation` is now complete through available validation as a frontend-only Reader enhancement step. The Reader Context Panel now adds secondary `Open in advanced Timeline / 고급 Timeline에서 보기` links only for supported related metadata targets: `event`, `book`, and `kingdom`. Related metadata chips themselves remain non-interactive, `place` items still have no Timeline link, and the panel continues to state that these links open current book-level preview metadata in the advanced Timeline validation view rather than implying selected-verse entity tagging or reader-side entity resolution. No Bible API / backend / DB / schema changes, Timeline data package row changes, place/genealogy/Korean-history deep links, or Timeline Kingdom extraction work were added.
