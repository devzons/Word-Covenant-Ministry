...
34. Recommended next CR: `CR-BR-CTX-68 Chapter Context Second Pilot Author Self-Check and Review-Required Promotion`. This should perform author self-check on the `6` second-pilot draft rows and, only if they pass, promote them from `draft` to `review-required` without changing the existing reviewed rows or Reader/runtime behavior.
