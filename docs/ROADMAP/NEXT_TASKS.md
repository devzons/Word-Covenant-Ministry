...
43. Recommended next CR: `CR-BR-CTX-77 Chapter Context Batch 3 Acceptance QA and Reviewed Promotion`. This should perform a separate acceptance pass on the same `6` Batch 3 rows and promote them to `reviewed` only if reviewer-acceptable wording, relation choices, and scope boundaries remain intact.
