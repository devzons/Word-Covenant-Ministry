...
1. `Scripture Evidence Panel Safe Completeness Fix Plan` is now documented in `docs/ROADMAP/SCRIPTURE_EVIDENCE_PANEL_SAFE_COMPLETENESS_FIX_PLAN.md`. This plan narrows the next implementation-safe scope to frontend-only and normalizer-only carry-through fixes that use existing source/package fields without adding rows, entities, APIs, schemas, coordinates, or external-reference layers. The first recommended implementation CR should cover Events, Books / Psalms, and Kings / Kingdoms only. Places panel clarity and broader wording standardization should remain separate follow-up CRs.
2. `Scripture Evidence Panel Modular Triage Plan` is now documented in `docs/ROADMAP/SCRIPTURE_EVIDENCE_PANEL_MODULAR_TRIAGE_PLAN.md`. The current audit result has now been translated into implementation-safe categories: rendering gaps, normalizer gaps, source-data gaps, intentional preview limitations, and approval-required expansion. The future right-side panel is being reframed as a modular context surface for the broader `Scripture Context Atlas`, but this triage step still authorizes no implementation. The next safe implementation-facing step is a narrow frontend-only completeness-fix plan focused on existing Events, Books, Kings, Places, and Genealogy panel data that already exists or can be safely carried through.
3. `Scripture Context Atlas Redesign Scope` is now documented in `docs/ROADMAP/SCRIPTURE_CONTEXT_ATLAS_REDESIGN_SCOPE.md`. The current Timeline workspace has a validated metadata baseline across Events, Books / Psalms, Kings / Kingdoms, Genealogy, Places, and supporting Korean-history references, but the product direction is now being reframed toward a Scripture-reading-first `Scripture Context Atlas`. The redesign keeps Bible text primary, reuses existing Timeline data as right-panel context supply, keeps map work schematic-only and no-coordinate, keeps original-language / paja work subordinate to Scripture context, and defers implementation. The next required step is `Scripture Evidence Panel Completeness Audit` before any reader-first layout prototype begins.
