...
16. Recommended next CR: `CR-BR-CTX-31 Chapter Context Verifier Relationship Rules Implementation`. This should be the next narrow verifier implementation CR. It should add field-specific validation for `relatedBookIds`, `relatedEventIds`, and `relatedKingdomIds`, keep empty arrays allowed, and continue to avoid any Reader runtime implication. It should continue to defer `relatedPlaceIds` unless a narrower source contract is explicitly approved inside that CR, and it should continue to defer chapter-context wording-warning implementation, production chapter rows, Reader runtime hookup, verse-level tagging, selected-verse-driven filtering, person/paja/original-language name atlas work, place/genealogy/Korean-reference deep links, map/coordinates/geocoding/provider work, Korean/world-history inspector integration, Timeline Kingdom extraction, and all API / backend / DB / schema / Timeline data package row changes.
