...
1. `Places panel basis/confidence clarity` is now complete through available validation. The Places / Schematic Map right-panel surface now presents existing `locationBasisLabel` and `locationConfidenceLabel` as explicit label/value rows rather than leaving them buried as note-only text, while `cautionNote` and `note` remain secondary caution/reference text. The place panel also now states its schematic-only, no-coordinate scope more structurally without introducing map-provider, geocoding, route reconstruction, deep-link, or package-migration work. No data rows, place package integration, API, DB, backend, schema, runtime import/export, or Bible-text rendering changes were added. The next recommended small frontend-only CR should focus on panel empty-state / preview-limitation wording standardization while person, paja/name, Korean/world-history inspector integration, prophet-context expansion, and map work remain deferred.
2. `Scripture Evidence Panel Completeness Fixes A` is now complete through available validation. The Events, Books / Psalms, and Kings / Kingdoms right-panel surfaces now preserve and render more existing package-backed metadata without adding data rows, entity types, APIs, schemas, coordinates, or external-reference layers. Existing source/package fields such as event confidence, event related kingdom/event IDs, book basis/confidence labels, book related book/kingdom IDs, and kings package basis metadata are now carried through and shown more clearly in the existing Scripture Evidence Panel structure. Bible text remains unrendered, chronology remains caution-labeled supporting metadata rather than exact proof, and no API, DB, backend, schema, or runtime import/export work was added.
3. `Scripture Evidence Panel Safe Completeness Fix Plan` is now documented in `docs/ROADMAP/SCRIPTURE_EVIDENCE_PANEL_SAFE_COMPLETENESS_FIX_PLAN.md`. This plan narrowed the implementation-safe scope to frontend-only and normalizer-only carry-through fixes using existing source/package fields without adding rows, entities, APIs, schemas, coordinates, or external-reference layers. Its first implementation step for Events, Books / Psalms, and Kings / Kingdoms is now complete; Places panel clarity is also now complete, while broader wording standardization and modular component refactoring remain separate follow-up CRs.
4. `Scripture Evidence Panel Modular Triage Plan` is now documented in `docs/ROADMAP/SCRIPTURE_EVIDENCE_PANEL_MODULAR_TRIAGE_PLAN.md`. The current audit result has now been translated into implementation-safe categories: rendering gaps, normalizer gaps, source-data gaps, intentional preview limitations, and approval-required expansion. The future right-side panel is being reframed as a modular context surface for the broader `Scripture Context Atlas`, but this triage step still authorizes no implementation. The next safe implementation-facing step after the current completeness fixes is a narrow frontend-only empty-state / preview-limitation wording pass followed later by modular component design.
5. `Scripture Context Atlas Redesign Scope` is now documented in `docs/ROADMAP/SCRIPTURE_CONTEXT_ATLAS_REDESIGN_SCOPE.md`. The current Timeline workspace has a validated metadata baseline across Events, Books / Psalms, Kings / Kingdoms, Genealogy, Places, and supporting Korean-history references, but the product direction is now being reframed toward a Scripture-reading-first `Scripture Context Atlas`. The redesign keeps Bible text primary, reuses existing Timeline data as right-panel context supply, keeps map work schematic-only and no-coordinate, keeps original-language / paja work subordinate to Scripture context, and defers implementation. The current small completeness passes for Events, Books, Kings, and Places are now complete; reader-first layout prototyping remains a later step.
