...
5. Recommended next CR: `CR-BR-CTX-20 Reader Context Panel Post-Link Scope Decision`. This should be a scope-decision step rather than immediate implementation, comparing small Reader-first options such as link-copy polish only, supported Timeline-link browser-regression checklist hardening, source/basis UI refinement, or chapter-level placeholder-only messaging, while continuing to defer verse-level tagging, person/paja/original-language name atlas work, map/coordinates/geocoding work, Korean/world-history inspector integration, Timeline Kingdom extraction, and any API / backend / DB / schema changes.
