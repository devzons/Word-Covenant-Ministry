...
42. Recommended next CR: `CR-BR-CTX-76 Chapter Context Batch 3 Author Self-Check and Review-Required Promotion`. This should perform a narrow author self-check on the same `6` Batch 3 rows and, only if explicitly approved, promote them from `draft` to `review-required` without changing package version, existing reviewed rows, or Reader/runtime scope.
