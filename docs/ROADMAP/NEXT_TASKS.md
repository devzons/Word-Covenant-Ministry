...
9. Recommended next CR: `CR-BR-CTX-24 Chapter Context Skeleton File Creation`. This should stay docs/data-only, create the first empty `docs/data-packages/timeline/chapter-context.skeleton.json` file using the approved envelope and `items: []` policy, and stop before any real chapter rows, Reader runtime connection, verifier implementation changes, verse-level tagging, selected-verse-driven filtering, person/paja/original-language name atlas work, place/genealogy/Korean-reference deep links, map/coordinates/geocoding/provider work, Korean/world-history inspector integration, Timeline Kingdom extraction, or API / backend / DB / schema / Timeline data package row changes begin.
