...
8. Recommended next CR: `CR-BR-CTX-23 Chapter Context Package Skeleton Design Note`. This should remain docs-only and translate the chapter-context contract into the smallest file-level skeleton policy needed before any real rows or Reader UI work begins. It should finalize package envelope wording, skeleton-file conventions, verifier expectation wording, fallback-copy guidance, and chapter-level review-state rules while continuing to defer verse-level tagging, selected-verse-driven metadata filtering, person/paja/original-language name atlas work, place/genealogy/Korean-reference deep links, map/coordinates/geocoding/provider work, Korean/world-history inspector integration, Timeline Kingdom extraction, and all API / backend / DB / schema / Timeline data package row changes.
