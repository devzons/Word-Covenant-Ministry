...
36. Recommended next CR: `CR-BR-CTX-70 Chapter Context Second Pilot Reader Route QA`. This should validate that the newly reviewed `6` rows are now eligible for Reader display through the existing reviewed-only loader path, confirm chapter-context visibility on the new pilot routes, and confirm fallback behavior remains unchanged without modifying Reader/runtime architecture.
