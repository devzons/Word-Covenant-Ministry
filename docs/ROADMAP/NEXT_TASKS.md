...
CR-90 Timeline Content Expansion Preview is now the active focus. The Scripture Timeline preview was expanded with a broader biblical storyline so the page can demonstrate period, book, place, density, Scripture anchor, Open in Reader, and selected-event detail behavior before staging/tag/release work resumes. Staging and release work remain paused until the expanded preview content model is visually satisfactory. CR-90 completed through available validation, including frontend typecheck, lint, build, `git diff --check`, and route smoke for `/ko/timeline` and `/en/timeline`.
