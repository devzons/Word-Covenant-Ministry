...
15. Recommended next CR: `CR-BR-CTX-30 Chapter Context Verifier Relationship and Warning Rule Readiness`. This should remain docs-only and compare the post-CR-BR-CTX-29 verifier behavior against the current chapter-context fixtures before a broader second implementation slice begins. It should explicitly examine the still-unenforced unresolved-related-id invalid fixture and the still-unwarned overconfident-wording warning fixture, then decide whether relationship-id validation and chapter-context wording warnings are safe to implement next without overreaching beyond the approved scope. It should continue to defer production chapter rows, Reader runtime hookup, verse-level tagging, selected-verse-driven filtering, person/paja/original-language name atlas work, place/genealogy/Korean-reference deep links, map/coordinates/geocoding/provider work, Korean/world-history inspector integration, Timeline Kingdom extraction, and all API / backend / DB / schema / Timeline data package row changes.
