...
35. Recommended next CR: `CR-BR-CTX-69 Chapter Context Second Pilot Acceptance QA and Reviewed Promotion`. This should inspect the `6` `review-required` second-pilot rows for final wording safety, relation conservatism, and acceptance readiness before any promotion to `reviewed`, while keeping the existing reviewed rows and Reader/runtime behavior unchanged.
