...
14. Recommended next CR: `CR-BR-CTX-29 Chapter Context Verifier Package Recognition and Envelope Rules`. This should be the first narrow verifier implementation CR. It should add `scripture-context-atlas.chapter-context` package recognition, chapter-context envelope validation, the approved skeleton empty-items special case, and the minimum safe chapter-row baseline while continuing to defer full relationship-id validation, expanded wording-warning logic, production chapter rows, Reader runtime hookup, verse-level tagging, selected-verse-driven filtering, person/paja/original-language name atlas work, place/genealogy/Korean-reference deep links, map/coordinates/geocoding/provider work, Korean/world-history inspector integration, Timeline Kingdom extraction, and all API / backend / DB / schema / Timeline data package row changes.
