...
12. Recommended next CR: `CR-BR-CTX-27 Chapter Context Verifier Fixture Files`. This should move from docs-only to docs/data-only and create the first chapter-context valid / invalid / warning fixture files under the existing `docs/data-packages/timeline/fixtures/` structure before any verifier implementation begins. It should continue to defer verifier code changes, real chapter rows, Reader runtime hookup, verse-level tagging, selected-verse-driven filtering, person/paja/original-language name atlas work, place/genealogy/Korean-reference deep links, map/coordinates/geocoding/provider work, Korean/world-history inspector integration, Timeline Kingdom extraction, and all API / backend / DB / schema / Timeline data package row changes.
