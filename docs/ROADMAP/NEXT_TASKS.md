...
40. Recommended next CR: `CR-BR-CTX-74 Chapter Context Batch 3 Draft Row Authoring`. This should add the approved `6` Batch 3 rows as draft-only chapter-context metadata, keep existing reviewed rows unchanged, and preserve the current reviewed-only Reader exposure path until later QA and promotion steps are complete.
