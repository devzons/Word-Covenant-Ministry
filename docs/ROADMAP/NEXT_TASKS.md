...
6. Recommended next CR: `CR-BR-CTX-21 Scripture Context Atlas Data Readiness Planning`. This should be a docs-only planning step that identifies the smallest next Reader-first expansion after Context Panel v1 closure, compares whether the next safe movement is book-context source/basis refinement, a chapter-level placeholder-only state, or a broader data-readiness plan for future Scripture Context Atlas entities, and keeps verse-level tagging, person/paja/original-language name atlas work, place/genealogy/Korean-reference deep links, map/coordinates/geocoding work, Korean/world-history inspector integration, Timeline Kingdom extraction, and all API / backend / DB / schema / Timeline data package row changes explicitly deferred.
