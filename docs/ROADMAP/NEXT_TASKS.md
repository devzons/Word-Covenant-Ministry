...
21. Recommended next CR: `CR-BR-CTX-52 Chapter Context Reader Loader Implementation`. This should implement only the minimal server-side loader and adapter wiring needed to make reviewed chapter-context rows available to the Reader page boundary without rendering them yet, while continuing to defer direct Reader UI display, runtime route hookup, verifier changes, fixture changes, verse-level tagging, selected-verse-driven filtering, person/paja/original-language name atlas work, place-package or map/coordinates/geocoding/provider work, Timeline Kingdom extraction, and all API / backend / DB / schema changes.
