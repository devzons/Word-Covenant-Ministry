...
44. Recommended next CR: `CR-BR-CTX-78 Chapter Context Batch 3 Reader Route QA`. This should confirm that the newly reviewed `6` Batch 3 rows are now eligible for Reader display through the existing reviewed-only loader path while fallback chapters still return `chapterContextPreview: null`, without changing Reader/runtime behavior.
