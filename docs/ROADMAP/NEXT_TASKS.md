...
17. Recommended next CR: `CR-BR-CTX-48 Chapter Context Pilot Draft Row QA`. This should inspect the current `4` draft pilot rows for content safety, verifier warnings, field consistency, and readiness for a later move from `draft` to a stricter review state. It should continue to defer Reader/runtime hookup, verifier code changes, fixture changes, verse-level tagging, selected-verse-driven filtering, person/paja/original-language name atlas work, place-package or map/coordinates/geocoding/provider work, Timeline Kingdom extraction, and all API / backend / DB / schema changes.
