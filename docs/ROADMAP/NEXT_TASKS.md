...
38. Recommended next CR: `CR-BR-CTX-72 Chapter Context Expanded Accessibility and Regression Spot Check`. This should perform a compact regression sweep across the expanded reviewed set, confirm the current supplemental display and fallback behavior remain stable after broader route exposure, and then hand off toward the next approved pilot-expansion decision without changing Reader/runtime architecture unless a concrete defect is found.
