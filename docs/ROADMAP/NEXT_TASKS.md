...
19. Recommended next CR: `CR-BR-CTX-50 Chapter Context Review-Required Acceptance QA`. This should perform the separate reviewer or acceptance pass on the current `4` `review-required` pilot rows, decide whether each row can move to `reviewed` or must remain `review-required`, and continue to defer Reader/runtime hookup, verifier code changes, fixture changes, verse-level tagging, selected-verse-driven filtering, person/paja/original-language name atlas work, place-package or map/coordinates/geocoding/provider work, Timeline Kingdom extraction, and all API / backend / DB / schema changes.
