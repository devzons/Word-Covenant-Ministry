...
CR-92C Korean History Pilot Rows is now complete as a docs/data package pilot step. `docs/data-packages/timeline/references.korean-pilot.json` now contains `5` manually curated broad Korean-history supporting-reference rows covering prehistoric and Bronze Age background, Gojoseon traditional reference, Han Commanderies, Proto-Three-Kingdoms / Samhan, and Three Kingdoms formation background. The pilot remains supporting-only, stores no Bible text, uses no coordinates, map-provider, or geocoding fields, adds no frontend integration, avoids exact biblical event synchronization, and keeps chronology caution-labeled rather than final. No frontend, verifier, API, DB, backend, schema, or runtime import/export behavior changed in this step.
```
