...
```txt
Use the documented CR-91C deep-link scope to implement query-restored metadata-only inspector selection for Events, Books / Psalms, and Kings / Kingdoms without adding Bible text, backend persistence, or authority-layer changes.
```

Next task:

```txt
Proceed to CR-91C-2 Events / Books / Kings Deep Link Implementation now that the CR-91C scope definition is documented.
```
