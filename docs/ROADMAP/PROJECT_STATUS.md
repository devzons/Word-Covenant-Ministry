...
Keep Timeline package-backed previews and accordion-first book navigation synchronized in roadmap documentation while preparing the next package-design branch.
```

Next task:

```txt
Proceed to CR-93G Kings / Kingdoms Timeline Package Design, followed by a skeleton-package step and a preview-integration step if approved.
```
