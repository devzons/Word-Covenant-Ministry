...
`CR-BR-CTX-48 Chapter Context Pilot Draft Row QA` is now complete as a QA-only review pass on the current `4` pilot draft rows. The QA result is pass with no row edits required, no verifier warnings, and no review-state changes. The current package remains `0.2.0-pilot` with all `4` rows still `reviewStatus: "draft"` and `isSkeleton: false`. No Reader/runtime hookup, verifier changes, fixture changes, API/backend/DB/schema changes, Bible-text storage/rendering changes, place-package work, or Timeline Kingdom extraction work were added. Timeline Kingdom extraction remains deferred.

`CR-BR-CTX-49 Chapter Context Draft-to-Review Gate Design` is now complete as a docs-only review-gate design step in `docs/ROADMAP/CHAPTER_CONTEXT_DRAFT_TO_REVIEW_GATE_DESIGN.md`. The project now has an explicit author self-check standard for when a chapter-context pilot row may move from `draft` to `review-required`, when it must remain `draft`, and when correction or blocker handling is required first. This design explicitly states that `review-required` means the author self-check has passed and the row is ready for a separate reviewer/acceptance CR, but it does not mean the row is reviewed, accepted, or runtime-ready. No chapter-context row content, no `reviewStatus` values, no package maturity, no Reader/runtime hookup, no verifier code, no fixture files, and no API / backend / DB / schema changes were added. Timeline Kingdom extraction remains deferred.
