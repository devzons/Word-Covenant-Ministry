...
```txt
Choose the next runtime-integration or verifier-automation branch after adding the package-level core biblical event skeleton.
```

Next task:

```txt
Choose between CR-93E-6 Verifier CI / command wiring, CR-93D-2 66-Book Frontend Preview Integration, and event-package integration planning before broadening Timeline fixture data further.
```
