...
`Personal Notes Responsive UI Polish` is now complete as an approved frontend-only Phase Exception within the still-active `CR-90Y` phase. Reader-side verse-note ownership has been moved out of the old inline `VerseNotePanel` into a shared `useVerseNoteWorkspace` controller that now owns selected verse reference, persisted note state, draft, dirty protection, loading, saving, deleting, error handling, login redirect, nonce refresh fallback, and logout/account reset behavior for both desktop and mobile presentations. The old large inline note editor under the active verse has been removed and replaced with a small per-verse trigger that stays visually quiet unless the verse is active or the row is hovered/focused. On desktop, note editing now opens inside the existing right-hand Scripture Research Workspace without changing Reader body height or introducing a new URL/query contract. On mobile, the same shared note state now renders through a compact bottom-sheet dialog with close/backdrop semantics and without auto-focusing the textarea. No Notes API, backend, DB, schema, Timeline package, or Meaning Layer state changed in this CR. `npm run typecheck` and `npm run lint` passed. Sandbox `npm run build` failed only with the known Turbopack process/port restriction, and an escalated `npm run build` passed successfully. Local route smoke to `wordcovenantministry.local:3030` still could not connect because no local dev server was listening during this session, so full browser/interactive verification remains the next gate. The next CR is `Personal Notes Responsive UI Browser QA and UX Polish`.

`Personal Notes Responsive UI Browser QA and UX Polish` is now complete as a narrow Reader follow-up polish step within the approved Notes Phase Exception. The verse-row note trigger is now icon-only, no longer renders the text labels `Note`, `노트`, or equivalent inline in the Reader flow, and is anchored to a quiet right-edge gutter with absolute positioning instead of consuming vertical space inside the verse text column. The current icon mapping now distinguishes three states without strong color: outline note for unknown/empty note state, note-with-lines for a verse whose saved note is already known in the current workspace session, and the same icon with a subtle selected background/border while that verse editor is open. The current spacing regression was corrected by removing the note trigger from the verse text column flow and restoring the text column to a direct verse-text stack with right padding reserved only for the gutter button, so the verse number and scripture text density return closer to the pre-notes Reader layout without changing backend/API behavior or reopening Timeline work. A follow-up stale active-verse icon bug was also fixed by making icon visibility depend on the current active verse plus current open note target rather than any previously opened workspace reference; verse-to-verse selection now keeps the note workspace target synchronized with the Reader active verse whenever the editor is already open, preserves dirty-state cancellation on the original verse, and guarantees only one trigger can carry the open-state treatment and `aria-expanded="true"` at a time. `npm run typecheck` and `npm run lint` passed. Sandbox `npm run build` again hit the known Turbopack process restriction and required an unrestricted build verification. Static code review confirms note API ownership remains centralized in `useVerseNoteWorkspace`, desktop still uses the right research panel, mobile still uses the compact sheet, and no Notes API/DB/schema change was introduced. Local browser interaction QA still remains partially limited by whether a dev server is actively running on `wordcovenantministry.local:3030`. The next CR is `Personal Notes Responsive UI Live Browser QA and Minor UX Follow-up`.
`Legal Notice Pages Foundation` is now complete as an approved frontend-only Phase Exception before registration work. Locale-aware static routes now exist for Korean and English Terms of Service and Privacy Policy at `/[locale]/terms` and `/[locale]/privacy`, backed by shared legal document metadata with `version: "1.0"` and `lastUpdated: "2026-07-26"`. Footer navigation now exposes both legal pages, and the login, forgot-password, and reset-password forms now include small legal links without adding any registration flow, consent checkbox, consent storage, or backend/API/DB/schema change. The legal draft content was limited to repository-confirmed facts: site name `Word Covenant Ministry`, documented production domain `https://wordcovenantministry.org`, account-based authentication, password reset flow, private verse notes, and repository-documented Bible/source-rights caution. No dedicated public privacy email address, postal address, phone number, confirmed production hosting provider, confirmed production email provider, fixed log-retention schedule, or governing-law statement could be verified from the repository, so those facts were not invented and remain legal-review gaps before production registration rollout. `npm run typecheck` and `npm run lint` passed, and build verification required the usual unrestricted run if the sandbox process restriction appeared. The next CR is `User Registration Backend Foundation`.
`User Registration Backend Foundation` is now complete as an approved backend-only Phase Exception. The existing `wcm/v1` auth controller now exposes `POST /auth/register`, `POST /auth/verify-email`, and `POST /auth/resend-verification` alongside the existing login/logout/me/password-reset routes. Registration validates normalized email, strict username, display name, password length and confirmation, locale (`ko` / `en`), and required Terms/Privacy acceptance before creating a WordPress user. New users are created through `wp_insert_user` with the current default role (falling back to `subscriber`), `wcm_requires_email_verification = 1`, legal consent metadata (`wcm_terms_accepted_version`, `wcm_terms_accepted_at`, `wcm_privacy_accepted_version`, `wcm_privacy_accepted_at`), and a one-time email verification token stored only as a SHA-256 hash plus expiry/sent timestamps in user meta. Verification now completes through a POST token exchange, resend always returns a generic public response to reduce enumeration, and unverified login is blocked only for users who explicitly carry the new verification-required meta so legacy/admin accounts remain compatible. Rate limiting is now applied with transient-backed IP/email/username buckets for registration, IP/email/account cooldown controls for resend, and IP-based verification throttling. No frontend registration UI, no automatic post-registration login, no Auth cookie contract change, and no backend DB/schema/table change were introduced. PHP syntax and Composer autoload validation passed, but local WordPress runtime validation remained blocked in this shell because `api.wordcovenantministry.local` was not reachable and local WP-CLI currently fails with Local PHP extension load errors plus `Error establishing a database connection.` The next CR is `User Registration Frontend Flow`.
`User Registration Frontend Flow` is now complete as an approved frontend-only Phase Exception on top of the existing registration backend contract. Locale-aware routes now exist for `/[locale]/register`, `/[locale]/register/check-email`, and `/[locale]/verify-email`, with Korean and English auth copy, client-side validation, required Terms/Privacy consent checkboxes linked to the existing legal pages, a shared auth API client for `register`, `verify-email`, and `resend-verification`, and safe internal login/register redirect preservation without external open redirects. Registration success now routes to a check-email screen without automatic login, verification is completed through an explicit one-click POST rather than auto-submitting on page load, resend keeps the backend’s generic public response semantics, and login now recognizes `email_verification_required` with a dedicated resend/check-email path instead of collapsing everything into a generic failure message. No auth cookie/session contract change, no backend/API redesign, no DB/schema change, no legal version trust from the client, and no Timeline or Meaning Layer change were introduced. `npm run typecheck`, `npm run lint`, and an unrestricted `npm run build` passed; direct local frontend/backend route smoke and full browser integration QA remained environment-limited from this shell because `wordcovenantministry.local:3030` and `api.wordcovenantministry.local` were unreachable here despite the frontend build succeeding. The next CR is `User Registration Integration and Security QA`.

`User Registration Integration and Security QA` is now complete as a runtime-backed validation and hardening step for the approved registration Phase Exception. On Sunday, July 26, 2026, the local frontend (`127.0.0.1:3030`), Local WordPress site (`api.wordcovenantministry.local`), WCM REST routes, WP-CLI runtime, and Mailpit API (`127.0.0.1:10011`) were all revalidated and used for live end-to-end QA. Korean and English registration both succeeded with `201` + `verification_required`, no automatic login, `subscriber` role assignment, `wcm_requires_email_verification = 1`, legal consent meta recorded at backend-owned version `1.0`, hash-only verification token storage, and localized verification mail delivery. Unverified login was blocked with `email_verification_required`, valid verification produced `verified: true` + `loginAllowed: true`, `/auth/me` remained unauthenticated until a later explicit login, verified login and logout succeeded, invalid/expired/replayed tokens failed safely, resend stayed publicly generic, and password reset still worked without colliding with verification token flow. Verified QA users were able to create/read/update/delete their own private verse notes and a second verified QA user still received `404` when attempting cross-user note access, confirming that registration/verification changes did not break existing note ownership rules. Two runtime defects were found and fixed in the same session: backend username validation now explicitly rejects characters outside `[A-Za-z0-9._-]` so WordPress cannot silently accept space-containing logins that the frontend already rejects, and `AuthRateLimiter` now coerces numeric transient values before incrementing so rate-limit counters persist correctly across requests instead of resetting whenever WordPress returns a numeric string. After a site-specific PHP-FPM reload, registration IP throttling now blocks the sixth repeated attempt with `registration_rate_limited`, verification IP throttling now blocks the twenty-first repeated invalid token attempt with `verification_rate_limited`, resend cooldown still suppresses duplicate delivery while keeping the public `accepted` response, and no raw password, raw token, client-controlled role, client-controlled legal version, or client-controlled verified state was introduced. All QA users created in this session were deleted after verification, QA notes were removed, and WCM auth transients were cleared. `php -l` checks for the auth layer, `composer dump-autoload`, `npm run typecheck`, `npm run lint`, and an unrestricted `npm run build` all passed. Because this shell still did not provide full live browser automation for desktop/mobile interaction, the registration Phase Exception is not closed yet; the next CR is `User Registration Final Browser QA`.
`User Registration Final Browser QA` is now complete as the final live browser gate for the approved registration Phase Exception. On Sunday, July 26, 2026, live headless Chrome interaction through the local Chrome DevTools Protocol was used against the actual frontend host (`wordcovenantministry.local:3030`), backend API host (`api.wordcovenantministry.local`), and Mailpit API (`127.0.0.1:10011`) to validate desktop and mobile auth flows at `1440 x 900`, `1024 x 768`, `390 x 844`, and `375 x 667`. Browser QA confirmed Korean and English registration page rendering, locale-specific login titles, required legal links opening out of the form flow, custom client validation with `aria-invalid`/`aria-describedby`, registration success routing to check-email without auto-login, generic resend behavior for pending and unknown addresses, localized verification emails and links, explicit one-click verification without auto-POST, invalid/missing token handling, pending-account login blocking, verified login plus authenticated `/auth/me`, logout, verified-user private note create/read/update/delete, `/my/notes`, and safe internal login/register return-path preservation while rejecting external or protocol-relative redirects. Three frontend browser defects were fixed in this CR: the login route was missing page metadata and now exports locale-aware login metadata; `RegisterForm` and `VerificationResendForm` now disable browser-native validation so custom locale-aware error handling is not preempted; and registration, resend, login, and verify actions now use in-flight ref locks so same-tick double-clicks cannot emit duplicate POST requests before React state disables the controls. The browser QA harness itself also needed a temporary React-compatible native value-setter path, but that harness change stayed in `/tmp` only and was not committed to the repository. Console inspection during the targeted browser checks showed no hydration warnings, no runtime exceptions, and no CORS failures on the exercised auth routes, while storage review confirmed no password, email, or raw token persistence in `localStorage` or `sessionStorage`, and no fake authenticated client state before verified login. QA users, QA notes, and WCM auth transients created during this browser pass were cleaned up after validation. With this CR, `User Registration and Email Verification Phase Exception Complete` is now the current closure state. The next CR is `User Registration Release Readiness and Production Configuration Review`.
